...
function post() {
    ...
    if (!isset($_POST['negocios_id'])) {
        echo json_encode(["error" => "Falta empresa_id"]);
        return;
    }

    $negocios_id = intval($_POST['negocios_id']);

    if (isset($_FILES['img']) && $_FILES['img']['error'] === UPLOAD_ERR_OK) {
        $img = file_get_contents($_FILES['img']['tmp_name']);

        $stmt = $conn->prepare("INSERT INTO negocios_imagenes (negocios_id, img) VALUES (?, ?)");
        if (!$stmt) {
        ...
        $null = NULL;
        $stmt->bind_param("ib", $negocios_id, $null);
